...
Das Paket \textit{org.woym.logic.util} enthält -- abgesehen von der Klasse \texttt{LimitedQueue} -- Klassen, welche Logikoperationen unter Verwendung anderer Klasse aus dem Paket \textit{org.woym.logic} für Klassen des Paktes \textit{org.woym.controller} und dessen Subpakete bereitstellen. %TODO: Erweitern
...
Das auf der folgenden Seite dargestellte Sequenzdiagramm zeigt den Ablauf des Hinzufügens eines Lesson-Objektes. Nach Aufruf von \textit{addLesson} wird zunächst die Methode \textit{validateActivity(Activity, TimePeriod)} auf dem \texttt{ActivityValidator} aufgerufen. Diese Methode prüft, ob die Aktivität den Stundenplänen von allen gewählten Teilnehmern hinzugefügt werden kann oder ob es dabei zu Überschneidungen kommt. Liegen keine Überschneidungen vor wird ein \texttt{SuccessStatus} zurückgegeben. Ist dies der Fall, wird ein neues \texttt{AddCommand} instanziiert und es der \textit{execute(ICommand)}-Methode des \texttt{CommandHandler} übergeben. Dieser ruft auf dem Command die \textit{execute}-Methode auf. Da der \texttt{CommandHandler} nicht weiß, was für ein konkretes Command er ausführt, ist der weitere Ablauf im Diagramm nicht dargestellt. In diesem Fall würde der \texttt{CommandHandler} ein \texttt{AddCommand} ausführen, welches die Methode \textit{persist(Entity)} der Klasse \texttt{DataAccess} aufruft.\\
Wird von dem \textit{execute}-Aufruf ein \texttt{SuccessStatus} zurückgegeben, wird das Command der Undo-Liste hinzugefügt und die Redo-Liste wird komplett geleert. Das vom Aufruf der \textit{execute}-Methode auf dem Command erhaltene \texttt{IStatus}-Objekt wird an den \texttt{LessonController} zurückgegeben. \\
Handelt es sich bei dem zurückgegebenen \texttt{IStatus}-Objekt um einen \texttt{SuccessStatus}, wird die \textit{init}-Methode des \texttt{TeacherController} aufgerufen.\\
...
Damit ist das Hinzufügen des \texttt{Lesson}-Objektes dann abgeschlossen.

\includepdf[landscape]{addLessonSequence.pdf}

%TODO: Zweites Diagramm
